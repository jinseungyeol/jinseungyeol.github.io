...
$.fn.smartPop = function(opt) {
	...
	var smartPop={
			open : function(options) {
                ...
                $box.show();
				$overlay.css({"width":document.body.scrollWidth+"px" , "height":document.body.scrollHeight+"px" ,"top":"0px" ,"left":"0px" , "position":"fixed", "background":"#000",  "opacity":"0.3" , "cursor":"pointer" , "pointer" : "auto","z-index":_smartPopupZIndex-1}).show();
				settings.w = smartPop.setSize(settings.initialWidth, 'x');
				settings.h = smartPop.setSize(settings.initialHeight, 'y');
				smartPop.position();
				$box.draggable({handle:'#cboxTitle'});
...
//			$wrap[0].style.width = $wrap[0].style.height = "9999px";
		...
		prep : function (object) {
			...
			callback = function () {
				...
				if(settings.title){
					var custom_title = $("<div class='pop_wrap'><div class='pop_header'><h1>"+settings.title+"</h1><a href='#none' onClick='close_smartPop();' class='btn_popclose'><img src='../img/btn/btn_popclose.gif' alt='팝업닫기'></a></div></div>");
					$title.html(custom_title).add($loaded).show();
					$title.css({top:"0px", textAlign:"left", height:"16px"});
				}else{
					$title.html(settings.title).add($loaded).show();
				}
