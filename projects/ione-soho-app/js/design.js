...
function cf_select_popup(title, hidden_id, text_id, code, callback) {
    ...
    var codeList = JexUtil.getCodeManagerByUsrSort().getCode(code);


    for (var key in codeList) {

        if (codeList[key].length > 24) {
            codeList[key] = codeList[key].substring(0, 25) + "<br>" + codeList[key].substring(25, codeList[key].length);
        }
